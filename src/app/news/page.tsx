...
const News = () => {
  ...
  return (
    <section className='flex relative flex-col container pt-[100px] text-black ' onMouseLeave={() => setShow(false)}>
      ...
      <div className='relative'>
        {
          ...
          posts.map((post: PostType) => (
            <div className='relative' onMouseEnter={() => GetPostItem({ post })} key={post.id}> // Wrap post in an object
      ...
      {
        !mobile && (
          <div className={`fixed z-50 right-0 -translate-x-1/2 translate-y-1/3 2xl:translate-y-1/4 perspective-800 ${show ? 'block transition-all duration-400 ease-in' : 'hidden'}`}>
